...
class CrazyflieServer:
    ...
    def ros_bridge_execute(self,Flag):
        ...
        if Flag == True:
            ...
            count = 0
        while(Flag):
            test_hight_cmd = np.sin(count * 1)*0.5
            if test_hight_cmd >= 1.5:
                test_hight_cmd = 1.5
            t_control = time.time()
            ...
            for cf in self.crazyflies:
                ...
                # if count % self.print_t ==0:
                #     print("Current pos_i: ",cf.object_name,current_pos_i)
                if mover_i[2] == 0: ## Hover (no cmd received or computing)
                    ## in optimization code, send zeros when computing to enable hover
                    # self.record("drone",f"{cf.object_name}-sp_SetPos", float(pos_i[0]), float(pos_i[1]), float(pos_i[2]))
                    ## x,y,z,yaw
                    # cf.cf.commander.send_position_setpoint(current_pos_i[0],current_pos_i[1],current_pos_i[2]+0.3,yaw_i)
                    # cf.cf.high_level_commander.go_to( 0.0, 0.0, 1.0, yaw=0.0, duration_s=1, relative=False, linear=False, group_mask=0 )
                    # cf.cf.commander.send_position_setpoint(0.0,0.0,test_hight_cmd,yaw_i)
                    # if (t_control - self.start_time)%10<=5:
                    #     cf.cf.commander.send_position_setpoint(0.0,0.0,0.25*2,yaw_i)
                    # else:
                    #     cf.cf.commander.send_position_setpoint(0.0,0.0,0.25*4,yaw_i)
                    # cf.cf.commander.send_position_setpoint(0.0,0.0,0.25*2,yaw_i)
            
                    
                    if t_elp <=15:
                        target_pos = np.array([0.0,0.0,0.6])
                    else:

                        target_list = np.array([[0.7,0.6,0.6],[-0.7,0.6,0.8],[-0.9,-0.1,0.7],[0.8,0.0,0.7]])
                        target_pos = np.array(target_list[wps_count,:])
                        r = LA.norm(target_pos-current_pos_i,2)
                        if r <= 0.2:
                            wps_count += 1
                            if wps_count >= 4:
                                wps_count = 0

                    vel_cmd = (target_pos - np.array(current_pos_i)) * 0.7
                    ...
                    self.data_history[cf.object_name][-1,10:13] = vel_cmd
                    # print("vel_cmd: ",vel)
                    if t_elp % 1 == 0:
                        print("time",t_elp)
                    cf.cf.commander.send_velocity_world_setpoint(vel_cmd[0], vel_cmd[1], vel_cmd[2], 0)
                    # if count == 0:
                    #     print("take off")
                    #     cf.cf.high_level_commander.takeoff( 0.5, 2.0 )
                    # elif np.floor(t_elp) % 4 == 0 and count != 0:
                    #     print("command 1")
                    #     cf.cf.high_level_commander.go_to( 0.0, 0.0, 1.0, yaw=0.0, duration_s=2.0, relative=False, linear=False, group_mask=0 )
                    # else:
                    #     print("command 2")
                    #     cf.cf.high_level_commander.go_to( 0.0, 0.8, 1.0, yaw=0.0, duration_s=2.0, relative=False, linear=False, group_mask=0 )
                    # cf.cf.commander.send_position_setpoint(0.0,0.0,0.25*3,0.0)
                    # cf.cf.commander.send_position_setpoint(0.0,0.0,test_hight_cmd+0.5,yaw_i)
                    # if count % self.print_t  ==0:
                    #     print("vel_cmd: ",vel)
                        # print("test hight", test_hight_cmd)
                        # print("Pos_cmd: ",cf.object_name , current_pos_i[0],current_pos_i[1],current_pos_i[2]+0.1)
                else:
                    self.record("drone",f"{cf.object_name}-sp_SetPos", float(pos_i[0]), float(pos_i[1]), float(pos_i[2]))
                    ## x,y,z,yaw
                    cf.cf.commander.send_position_setpoint(pos_i[0],pos_i[1],pos_i[2],yaw_i)
                    if count % self.print_t  ==0:
                        print("Pos_cmd: ", cf.object_name ,pos_i[0],pos_i[1],pos_i[2])
            count += 1
    ...
    def logging( self ):
        while self.is_open:
            ...
            for cf in self.crazyflies:
                ...
                ## add new data
                self.data_history[name][data_length-1][0:10] = current_data_i


                # if self.record_count_ekf % 50 == 0:
                #     print("EKF euler:   ",euler)
                #     print("EKF q:    ",state[3:7])
                self.record( "kalman-" , cf.object_name, state[0], state[1],state[2],state[3],state[4],state[5],state[6])
